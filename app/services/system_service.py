...
def count_images_today(images_dir):
    ...
    today_dir = os.path.join(
        images_dir, str(today.year), f"{today.month:02d}", f"{today.day:02d}"
    )
    try:
...
def get_quick_stats():
    ...
    return {
        "cpu_temp": get_cpu_temperature(),
        "disk_percent": (
            get_disk_usage("/").get("percent") if get_disk_usage("/") else None
        ),
        "memory_percent": (
            get_memory_usage().get("percent") if get_memory_usage() else None
        ),
